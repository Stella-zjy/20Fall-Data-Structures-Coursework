...
class Tree:
    ...
    def has_duplicate(self):
        ...
        """
        # Task 1      
        def helper1(values, node):
            if node == None:
                return False
            elif node._element in values:
                return True
            else:
                values.append(node._element)
            
            return helper1(values, node._left) or helper1(values, node._right)
        
        
        return helper1([], self._root)
    ...
    def is_height_balanced(self):
        ...
        def helper2(node):
            if node == None:
                return -1
            elif abs(helper2(node._left) - helper2(node._right)) > 1:
                return False
            else:
                return max(helper2(node._left), helper2(node._right)) + 1
        
        if helper2(self._root) == False:
            return False
        return True
    ...
    def sum_of_leaves(self):
        ...
        def helper3(node):
            ...
            return helper3(node._left) + helper3(node._right)
        
        return helper3(self._root)
    ...
    def evaluate(self):
        ...
        def helper6(node):
            if type(node._element) == int:
                return node._element
            else:
                return calculate(helper6(node._left), node._element, helper6(node._right))
            
        return helper6(self._root)
...
def is_isomorphic(tree1, tree2):
    ...
    """
    # Task 4
    def helper4(node1, node2):
        if node1 == node2 == None:
            return True
        elif node1 == None or node2 == None:
            return False
        elif node1._element != node2._element:
            return False
        else:
            return helper4(node1._left, node2._left) and helper4(node1._right, node2._right)\
                or helper4(node1._right, node2._left) and helper4(node1._left, node2._right)              
           
    return helper4(tree1._root, tree2._root)


    # def helper4(tree1, node1, tree2, node2):
    #     if node1 == node2 == None:
    #         return True
    #     elif node1 == None or node2 == None:
    #         return False
               
    #     l1 = [i._element for i in tree1.children(node1)]
    #     l2 = [i._element for i in tree2.children(node2)]
    #     s1 = set(l1)
    #     s2 = set(l2)
    ...
    #         return False
    #     else:
    #         if node1._left == node2._left:
    #             return helper4(tree1, node1._left, tree2, node2._left)\
    #                 and helper4(tree1, node1._right, tree2, node2._right)
    #         else:
    #             return helper4(tree1, node1._right, tree2, node2._left)\
    #                 and helper4(tree1, node1._left, tree2, node2._right)
           
    # return helper4(tree1, tree1._root, tree2, tree2._root)
...
def build_expression_tree(postfix):
    ...
    tree = Tree()
    root = tree.add_root(l.pop())
    previous = root
    
    while l != []:
        if type(previous._element) != int and previous._right == None:
            new = tree.add_right(previous, l.pop())
            previous = new
        else:
            if type(previous._element) == int:
                previous = previous._parent
                while previous._left != None:
                    previous = previous._parent
                new = tree.add_left(previous, l.pop())
            else:
                new = tree.add_left(previous, l.pop())
            if type(new) != int:
                previous = new
