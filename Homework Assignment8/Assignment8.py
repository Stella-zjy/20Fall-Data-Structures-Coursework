...
class BinarySearchTree(Tree):
    ...
    def largest_to_smallest(self):
        ...
        def helper1(current, output, index):
            if current._right is not None:
                output, index = helper1(current._right, output, index)
            output[index] = current._element
            index += 1
            if current._left is not None:
                output, index = helper1(current._left, output, index)
            return output, index
      
        output, index = helper1(self._root, output, index)
        return output
    
    
        # output = []
        
        # def helper1(current, output):
        #     if current._right is not None:
        #         helper1(current._right, output)
        #     output.append(current._element)
        #     if current._left is not None:
        #         helper1(current._left, output)
        
        # helper1(self._root, output)
        # return output
    ...
    def get_kth_largest(self, k):
        ...
        if k < 1:
            while x._right is not None:
                x = x._right
            return x
        elif k > num:
            while x._left is not None:
                x = x._left
            return x
        else:
            count = 0
